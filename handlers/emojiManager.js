...
const STOP_WORDS = new Set([
  'برای','یه','یک','یه‌','ایموجی','ایموجیی','مدل','مدلها','مدل‌ها',
  'نشون','نشان','بیار','بده','پیدا','پیداش','کن','کنه','مناسب',
  'خفن','خوب','بهترین','مختلف','چندتا','چند','تا','عدد','نوع','انواع'
]);
...
function buildIndex() {
  index = library.map((item, i) => {
    ...
    return {
      i, emoji, setName, id,
      search: normalizeText(`${item.emoji || ''} ${item.set_name || ''} ${id}`)
    };
  });
}
  ...
  for (const token of tokens) {
    ...
    if (token.length >= 3) {
      ...
      for (const word of words) {
        ...
        if (distance <= Math.max(1, Math.floor(maxLen * 0.25))) {
          score += 180;
          break;
        }
      }
...
function searchLibrary(query = '', limit = MAX_RESULTS) {
  ...
  if (results.length < limit) {
    const used = new Set(results.map(x => x.custom_emoji_id));
...
function renderCustomEmoji(item) {
  if (!item || !item.custom_emoji_id) return '❓';
...
async function getAssignment(name) {
  const assignments = await getAssignments();
  return assignments[normalizeText(name)] || null;
}

async function getAssignedEmojiId(name) {
  const assignment = await getAssignment(name);
  return assignment ? assignment.custom_emoji_id : null;
}
...
async function showManager(ctx, text = null) {
  ...
  await ctx.editMessageText(body, {
    ...
    reply_markup: {
      inline_keyboard: [
        [{ text: '🔎 جستجوی آزاد', callback_data: 'emoji_mgr_search' }],
        [{ text: '🎲 انتخاب تصادفی', callback_data: 'emoji_mgr_random' }],
        [{ text: '💾 موارد ذخیره‌شده', callback_data: 'emoji_mgr_saved' }],
...
async function showResults(ctx, results, title = 'نتایج') {
  if (!results.length) {
    await ctx.reply('❌ موردی پیدا نشد.');
    return;
  }
  ensureSession(ctx).emojiManager = {
  ...
  rows.push([
    { text: '🔎 جستجوی جدید', callback_data: 'emoji_mgr_search' },
    { text: '🎨 مدیریت', callback_data: 'admin_emoji_manager' }
  ]);
  const preview = results
    .slice(0, 20)
    .map((item, i) => `${i + 1}. ${renderCustomEmoji(item)}`)
    .join('  ');
  await ctx.reply(
    `🎨 <b>${title}</b>\n\n` +
    `${preview}\n\n` +
...
module.exports = function registerEmojiManager(bot) {
  ...
  bot.action('emoji_mgr_search', async ctx => {
    if (!isAdmin(ctx.from.id)) {
      return ctx.answerCbQuery('⛔ دسترسی ندارید', { show_alert: true });
    }
    await ctx.answerCbQuery().catch(() => {});
    ensureSession(ctx).emojiManager = { mode: 'search' };
    await ctx.reply(
      `🔎 <b>جستجوی آزاد</b>\n\n` +
  ...
  bot.action('emoji_mgr_random', async ctx => {
    if (!isAdmin(ctx.from.id)) {
      return ctx.answerCbQuery('⛔ دسترسی ندارید', { show_alert: true });
    }
    const results = searchLibrary('', MAX_RESULTS);
    await ctx.answerCbQuery('🎲 آماده شد');
    await showResults(ctx, results, '🎲 انتخاب تصادفی');
  });

  bot.action('emoji_mgr_saved', async ctx => {
    if (!isAdmin(ctx.from.id)) {
      return ctx.answerCbQuery('⛔ دسترسی ندارید', { show_alert: true });
    }
    const assignments = await getAssignments();
    ...
    await ctx.answerCbQuery().catch(() => {});
    if (!entries.length) {
      return ctx.reply('💾 هنوز هیچ Custom Emoji با نام دلخواه ذخیره نشده است.');
    }
    const lines = entries
      .slice(0, 100)
      .map((x, i) => `${i + 1}. ${renderCustomEmoji(x)} <b>${x.name}</b>`);
    await ctx.reply(
      `💾 <b>موارد ذخیره‌شده</b>\n\n` + `${lines.join('\n')}`,
      {
        parse_mode: 'HTML',
  ...
  bot.action(/^emoji_pick_(\d+)$/, async ctx => {
    if (!isAdmin(ctx.from.id)) {
      return ctx.answerCbQuery('⛔ دسترسی ندارید', { show_alert: true });
    }
    const indexNumber = Number(ctx.match[1]);
    const manager = ctx.session?.emojiManager || {};
    const item = manager.results?.[indexNumber];
    if (!item) {
      return ctx.answerCbQuery('❌ این نتیجه دیگر موجود نیست', { show_alert: true });
    }
    ensureSession(ctx).emojiManager = {
      mode: 'assignment_name',
      selected: item
    };
    await ctx.answerCbQuery('✅ انتخاب شد');
    await ctx.reply(
      `🎯 انتخاب شد:\n\n` +
  ...
  bot.on('text', async (ctx, next) => {
    ...
    const state = ctx.session?.emojiManager;
    if (!state?.mode) return next();
    const text = String(ctx.message.text || '').trim();
    if (!text) return;

    if (state.mode === 'search') {
      const results = searchLibrary(text, MAX_RESULTS);
      ensureSession(ctx).emojiManager = { mode: 'results', results };
      return showResults(ctx, results, `نتایج برای «${text}»`);
    }

    if (state.mode === 'assignment_name') {
      if (!state.selected) {
        clearMode(ctx);
        return ctx.reply('❌ ایموجی انتخاب‌شده پیدا نشد.');
      }
      const saved = await saveAssignment(text, state.selected);
      clearMode(ctx);
      return ctx.reply(
        `✅ <b>ذخیره شد</b>\n\n` +
    ...
    await ctx.reply(
      ...
      `📚 ${library.length.toLocaleString('en-US')} ایموجی در کتابخانه موجود است.`,
      {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: '🔎 جستجوی آزاد', callback_data: 'emoji_mgr_search' }],
            [{ text: '🎲 تصادفی', callback_data: 'emoji_mgr_random' }],
            [{ text: '💾 ذخیره‌شده‌ها', callback_data: 'emoji_mgr_saved' }]
          ]
        }
      }
    );
...
module.exports.getAssignedEmojiId = getAssignedEmojiId;
module.exports.renderCustomEmoji = renderCustomEmoji;
module.exports.searchLibrary = searchLibrary;
