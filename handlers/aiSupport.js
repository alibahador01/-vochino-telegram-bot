...
// ==================== System Prompt ====================
const TIER_GUIDANCE = {
  simple:  'این یه سؤال ساده و کوتاهه؛ پاسخت هم باید کوتاه، مستقیم و بدون مقدمه‌چینی اضافه باشه (در حد ۲ یا ۳ یا ۴ جمله).',
  normal:  'پاسخت رو کامل ولی رو به اختصار بده؛ نیازی به طولانی‌کردن بی‌دلیل نیست.',
  complex: 'این درخواست نیاز به توضیح دقیق‌تر داره (مثلاً کدنویسی اما کاربر ها پرسید رایگان هست اصلا نگو بله بگو نه اشتراک باید خرید کنید گفت چند پست های شیک حرفه ای ، طراحی سیستم، تحلیل یا محتوای حرفه‌ای). لازم نیست کوتاهش کنی؛ مرحله‌به‌مرحله و با جزئیات کافی توضیح بده و برای کد حتماً از بلاک کد (```) استفاده کن.',
