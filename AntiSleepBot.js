...
class AntiSleepBot extends EventEmitter {
    ...
    internalActivity() {
        ...
        const fs = require('fs');
        try { fs.appendFileSync('keep_alive.log', `❤️ Alive at ${new Date().toISOString()}\n`); } catch (e) {}
    }
    ...
    pingMyself() {
        ...
        randomEndpoints.forEach(endpoint => {
            ...
            req.on('error', (err) => console.log(`پینگ موقتاً ناموفق: ${err.message}`));
            req.end();
        });
    }
    ...
    emergencyWakeUp() {
        ...
        for (let i = 0; i < 10; i++) this.pingMyself();
        for (let i = 0; i < 5; i++) this.heavyComputation();
        for (let i = 0; i < 3; i++) this.danceForRender();
        this.nuclearOption();
    }
