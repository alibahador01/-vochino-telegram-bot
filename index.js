...
// ✅ جدید: مدیریت محصولات فروش (کاربر می‌فروشه، ما می‌خریم)
// فرمت: /addsellproduct کلید|نام نمایشی|قیمت واحد (تومان)|نمونه کد صحیح
...
// ✅ جدید: منوی فروش (کاربر می‌فروشه به ما)
...
bot.on('text', async (ctx, next) => {
  ...
  // ✅ جدید: دریافت کد ووچر فروش از کاربر
  ...
  // ✅ جدید: ادمین دلیل رد فروش رو می‌فرسته
  ...
  // ✅ جدید: ادمین مبلغ نهایی فروش رو وارد می‌کنه
  if (session.flow === 'admin_sell_amount' && session.step === 'waiting_amount') {
    ...
    bot.telegram.sendMessage(
      request.telegram_id,
      fillTemplate(t.sellApprovedUser, {
...
// ✅ جدید: لیست سفارش‌های فروش در انتظار بررسی مالی
...
async function init() {
  ...
  // ✅ جدید: جدول محصولات فروش (کاربر می‌فروشه، ما می‌خریم)
  await pool.query(
  ...
  // ✅ جدید: مقدار اولیه‌ی محصولات فروش (طبق نمونه‌هایی که خودت دادی)
