...
async function showAdminMenu(ctx) {
  ...
  ctx.reply('👑 پنل مدیریت پیشرفته\n\n' +
    ...
    '/activateproduct کلید — فعال و قابل نمایش کردن محصول\n' +
...
module.exports = function registerAdminHandlers(bot) {
  ...
  bot.command('addproduct', async (ctx) => {
    ...
    await pool.query(
      ...
      'ON CONFLICT (key) DO UPDATE SET name = EXCLUDED.name, min_amount = EXCLUDED.min_amount, max_amount = EXCLUDED.max_amount, price_type = EXCLUDED.price_type, delivery_type = EXCLUDED.delivery_type, active = 1, hidden = 0',
  ...
  bot.command('activateproduct', async (ctx) => {
    ...
    const args = ctx.message.text.split(' ');
    if (args.length < 2) {
      ctx.reply('❌ کلید محصول رو وارد کنید.\nمثال:\n/activateproduct voucher');
      return;
    }
    const key = args[1].trim();
    const res = await pool.query("UPDATE products SET active = 1, hidden = 0 WHERE key = $1 RETURNING name", [key]);
    if (res.rows.length === 0) {
      ctx.reply('محصولی با این کلید پیدا نشد.');
      return;
    }
    ctx.reply('✅ محصول «' + res.rows[0].name + '» فعال شد و دوباره به کاربران نمایش داده می‌شه.');
  });
